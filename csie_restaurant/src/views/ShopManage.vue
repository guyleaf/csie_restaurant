  ...
  created(){
    ...
    this.$http.get('restaurants/'+id+'/Info')
        .then(response => {
          ...
          this.Info.push({description: data.description, joinDate: data.created_at.split(" ",1)[0],shopName: data.name, imgPath:"https://picsum.photos/900/250/?image=3", rate:Math.round(parseFloat(data.averageOfRatings)*10)/10, numberOfRatings:parseInt(data.numberOfRatings), fans:parseInt(data.numberOfFans)});
        })
  }
};
</script>
